...
## Run Execution & Eventing (execution split)

Every run flows through one seam: `startRunServer()` (`src/server/runner.ts`) =
`prepareRun()` (setup — agent load, per-agent guard, workspace/worktree, `runs`
row, MCP config) + an `Executor.execute()` (spawn/stream/finalize). The executor
is selected by **`CONDUIT_EXECUTOR`** (`inproc` default | `job`) — see
`src/server/executor.ts`. `InProcessExecutor` (in `runner.ts`) is today's
child-process path; `JobExecutor` (per-run Kubernetes Job dispatch) is a stub
pending the follow-on (P3 changes B/C/E/F/G).

**Run coordination is RDS-backed, not an in-process Map** (so it is multi-pod-safe):
- Per-agent concurrency guard = `hasActiveRunForAgent()` in
  `src/main/db/queries/runs.ts` (a `runs` query), NOT the `activeProcesses` Map.
  That Map stays pod-local, feeding only the data-dir sweeper
  (`getActiveWorkspacePaths`/`getActiveRunIds` — worktrees are pod-local).
- Live events + durable logs live in the `run_events` table
  (`src/main/db/queries/runEvents.ts`): the runner mirrors every event there
  (batched, capped like the on-disk log via `CONDUIT_RUN_LOG_MAX_BYTES`).
  `runs:getLog`/`readRunLog` (`src/server/utils.ts`) read RDS first, falling back
  to `/data/logs/<runId>.jsonl` for historical runs. `runs.executor` /
  `pod_name` / `heartbeat_at` record how/where a run executes.
- `RunEventBus` (`src/server/runEventBus.ts`) `LISTEN`s on Postgres
  `NOTIFY run_events` and re-broadcasts to WS clients — the fan-out for
  out-of-process (Job) runs. In-process runs broadcast directly and do **not**
  `NOTIFY` (no double-delivery), so the bus is inert until `CONDUIT_EXECUTOR=job`.
